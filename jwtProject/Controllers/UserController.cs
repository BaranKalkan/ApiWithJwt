...
namespace jwtProject.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> UserFavBooks()
        {
            ...
            var BookList = new List<FavBook>();
            await _apiDbContext.AllFavbooks.Include(x => x.book).ForEachAsync<FavBook>(x =>
            {
        ...
        public async Task<IActionResult> CurrentPage(int BookId)
        {
            ...
            if (current == -1)
            {
                return BadRequest(new GeneralResponse()
                {
                    Errors = new List<string>()
                        {
                            "Couldn't changed the book page!"
                        },
                    Success = false,
                });
            }
        ...
        public async Task<IActionResult> CurrentPage(int BookId, int Current)
        {
            ...
            if (success==false)
            {
                return BadRequest(new GeneralResponse()
                {
                    Errors = new List<string>()
                        {
                            "Book id cannot found in UserBook!"
                        },
                    Success = false,
                });
            }
